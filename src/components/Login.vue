    ...
    <el-tabs :stretch="true">
      <el-tab-pane label="登录">
        <el-form class="login-container" ref="login" :model="login">
          ...
          <el-form-item label="">
            <el-input v-model="login.user_password" placeholder="密码" show-password></el-input>
          </el-form-item>
          <el-form-item>
...
import * as UserAPI from "@/APIs/user.js";
  ...
  export default {
    ...
    methods: {
      async doSubmitLogin() {
        try {
          const userInfo = await UserAPI.login(this.login.username, this.login.user_password);
      ...
      async doSubmitRegister(formName) {
        this.$refs[formName].validate(async (valid) => {
          if (valid) {
            try {
			const userInfo = await UserAPI.register(this.register.username, this.register.user_password, this.register.user_nickname, this.register.school, this.register.school_id, this.register.email);
			const res = userInfo.data.msg;
